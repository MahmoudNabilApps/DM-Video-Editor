...
internal fun VideoEditingActivity.showTextSheet(editIdx: Int?) {
    ...
    val btnDone = v.findViewById<android.widget.Button>(R.id.btnDoneText)
    ...
    var selAnim   = existing?.animationType ?: "none"

    // Animation selection
    val animContainer = LinearLayout(this).apply {
    ...
    TextView(this).apply {
        text = "🎬 حركة النص (Animation)"; textSize = 13f; setTextColor(Color.parseColor("#AAAAAA"))
        setPadding(0, 0, 0, 4)
    }.also { animContainer.addView(it) }

    val rvAnim = RecyclerView(this).apply {
        layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, (100 * resources.displayMetrics.density).toInt())
        layoutManager = LinearLayoutManager(this@showTextSheet, LinearLayoutManager.HORIZONTAL, false)
    }
    rvAnim.adapter = TextAnimationAdapter(TextAnimationType.getAll(), selAnim) { selAnim = it.code }
    animContainer.addView(rvAnim)

    // Add anim container before Done button
    val parentLayout = btnDone.parent as? ViewGroup
    parentLayout?.addView(animContainer, parentLayout?.indexOfChild(btnDone) ?: 0)
    ...
    parentLayout?.addView(styleContainer, parentLayout?.indexOfChild(btnDone) ?: 0)
    ...
    parentLayout?.addView(btnTime, parentLayout?.indexOfChild(btnDone) ?: 0)
